...
public abstract class Document2 {
	...
	protected int countSyllables(String word)
	{
		...
		if (!w.endsWith("e")) {
			return count(w);
		} else if (!w.substring(0, w.length() - 1).isEmpty()) {
			w = w.substring(0, w.length() - 1);
			if (checkLone(w)) return 1;
			return count(w);
		} else {
			return 1;
		}
...
//	    return 0;
	...
	private boolean checkLone(String w) {
	...
	private int count(String w) {
		...
		for (char c : w.toCharArray()) {
            if ((l.isEmpty() || !dv(l.getLast())) && dv(c) || !dv(c)) {
                l.add(c);
            }
        }
		return l.stream().filter(o -> dv(o)).collect(Collectors.toList()).size();
	}


	private boolean dv(char c) {
	...
	{
		double wds = getNumWords();
		double sen = getNumSentences();
		double syl = getNumSyllables();
		return 206.835 - 1.015 * (wds / sen) - 84.6 * (syl / wds);
