...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		numWords = (int) tokens.stream().filter(e -> isWord(e)).count();
		numSentences = (int) tokens.stream().filter(e -> !isWord(e)).count();
		if (getText().matches("^.*?[^!?.]$"))
			numSentences++;
		numSyllables = (int) tokens.stream().map(e -> countSyllables(e)).mapToInt(Integer::intValue).sum();
